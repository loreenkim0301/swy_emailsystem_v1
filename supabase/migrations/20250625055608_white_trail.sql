/*
  ...
  1. 새로운 데이터
    - 웹사이트 카테고리: 7개 항목 (출시됨 2개, 출시 예정 5개)
    - 블로그 카테고리: 4개 항목 (모두 공개됨)
  
  2. 데이터 구조
    - ID 충돌 방지를 위해 100번대부터 시작
    - 시퀀스 값 업데이트로 향후 자동 생성 ID 충돌 방지
    
  3. 카테고리 분류
    - website: 학습 도구 및 웹 애플리케이션
    - blog: 블로그 포스트 및 튜토리얼
*/
...
-- 105. Bolt.new로 관리자 대시보드 디자인하기 (예정)
(
  105,
...
-- 106. Bolt.new로 데이터베이스의 CRUD 연습하기 (예정)
(
  106,
...
-- 시퀀스 값 업데이트 (향후 자동 생성되는 ID가 기존 데이터와 충돌하지 않도록)
SELECT setval('blogs_id_seq', 300, false);